...
# cd ../input 
# ln -s /Volumes/LACIE_SHARE/data/2020-06-10-10x-scATAC-seq/atac_pbmc_5k_nextgem_possorted_bam/atac_pbmc_5k_nextgem_possorted_bam.bam ./    
# samtools view ./atac_pbmc_5k_nextgem_possorted_bam.bam -H > atac_nextgem_pbmc_5k_possorted_bam.header.sam

#Create a bam file with the barcode embedded into the read name
# cat <( cat atac_nextgem_pbmc_5k_possorted_bam.header.sam ) \ <( samtools view atac_pbmc_5k_nextgem_possorted_bam.bam | awk '{for (i=12; i<=NF; ++i) { if ($i ~ "^CB:Z:"){ td[substr($i,1,2)] = substr($i,6,length($i)-5); } }; printf "%s:%s\n", td["CB"], $0 }' ) \ | samtools view -bS - > 10xpbmc5k.snap.bam
